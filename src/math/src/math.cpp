...
namespace math
{
    ...
    const real
    Lerp(const real f, real scale, const real t)
    {
        return f + (t - f) * scale;
    }

    const real
    Curve(const real newValue, const real oldValue, const real smoothingValue)
    {
    ...
    const real
    Min(const real lhs, const real rhs)
    {
        if (lhs < rhs)
        {
            return lhs;
    ...
    const real
    Max(const real lhs, const real rhs)
    {
        if (lhs > rhs)
        {
            return lhs;
    ...
    const real
    To01(const real L, const real v, const real U)
    {
        return (v - L) / (U - L);
    }

    const real
    From01(const real L, const real v, const real U)
    {
        return v * (U - L) + L;
    }
    ...
    const real
    Get360Angular(const real min, const real value, const real max)
    {
        const real halfdiff = (max - min) / PWN_MATH_VALUE(2.0);
        return min + halfdiff -
                halfdiff *
                Cos(Angle::FromDegrees(value * PWN_MATH_VALUE(360.0)));
    }

    const real
    KeepWithin(const real min, const real v, const real max)
    {
        if (v > max)
        {
            return max;
    ...
    const bool
    IsWithin(const real min, const real c, const real max)
    {
        return c > min && c < max;
    }

    const bool
    IsWithinInclusive(const real min, const real c, const real max)
    {
        return c >= min && c <= max;
    }

    const real
    Wrap(const real min, const real v, const real max)
    {
    ...
    {
        real
        LowerBound(real num, real gran)
        {
            return std::floor(num / gran) * gran;
        }

        real
        UpperBound(real num, real gran)
        {
    ...
    const real
    Round(const real num, const real gran)
    {
        const real lower = LowerBound(num, gran);
        const real upper = UpperBound(num, gran);
        const real percent = (num - lower) /
                gran;  // 1.0 is upper, 0.0 is lower, 0.25 is 25% between lower and upper, moving from lower to upper
