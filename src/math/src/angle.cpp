...
namespace math
{
    ...
    {
        const real
        rad2deg(const real rad)
        {
            return (PWN_MATH_VALUE(180.0) / Pi()) * rad;
        }
        const real
        deg2rad(const real deg)
        {
    ...
    const Angle
    Angle::FromDegrees(real deg)
    {
        return Angle(deg2rad(deg));
    }

    const Angle
    Angle::FromRadians(real rad)
    {
        return Angle(rad);
    }

    Angle::Angle(real rad) : mRad(rad)
    {
    }
    ...
    {
        Assert(v <= 1 &&
               "v must be smaller than 1, use Limmit or Max on the value to not trigger this Assert");
        Assert(v >= -1 &&
               "v must be greater than -1, use Limmit or Min on the value to not trigger this Assert");
    ...
    const Angle
    operator*(const Angle& lhs, const real rhs)
    {
        Angle temp(lhs);
        temp *= rhs;
        return temp;
    ...
    const Angle
    operator*(const real rhs, const Angle& lhs)
    {
        return lhs * rhs;
    }

    const Angle
    FromPercentOf360(const real percent)
    {
    ...
    std::ostream&
    operator<<(std::ostream& s, const Angle& a)
    {
        s << a.inDegrees() << "°";
        return s;
    }
    ...
    std::ostream&
    operator<<(std::ostream& s, const AxisAngle& aa)
    {
