...
namespace math
{
    ...
    const vec3
    GetNormalized(const vec3& vec)
    {
        if (vec.length_squared() < 0.001f)
        {
            return vec;
        }
        vec3 temp = vec;
    ...
    const vec3
    crossNorm(const vec3& lhs, const vec3& rhs)
    {
        return GetNormalized(cross(lhs, rhs));
    }

    vec3
    Curve(const vec3& target, const vec3& old, real smoothing)
    {
        return vec3(
                Curve(X(target), X(old), smoothing),
                Curve(Y(target), Y(old), smoothing),
                Curve(Z(target), Z(old), smoothing));
    }

    const vec3
    FromTo(const vec3& from, const vec3& to)
    {
        return to - from;
    }

    const vec3
    Lerp(const vec3& f, real scale, const vec3& t)
    {
        return f + (t - f) * scale;
    }

    const vec3
    Qlerp(const vec3& f, real scale, const vec3& t)
    {
    ...
    const uint16
    UnitVectorToCompressed(const vec3& vec)
    {
        ...
        if (X(tmp) < 0)
        {
            compressed |= unit::XSIGN_MASK;
            X(tmp) = -X(tmp);
        }
        if (Y(tmp) < 0)
        {
            compressed |= unit::YSIGN_MASK;
            Y(tmp) = -Y(tmp);
        }
        if (Z(tmp) < 0)
        {
            compressed |= unit::ZSIGN_MASK;
            Z(tmp) = -Z(tmp);
        }
    ...
    const vec3
    CompressedToUnitVector(uint16 compressed)
    {
        ...
        float32 uvadj = unit::CompressedConstants()
                                .mUVAdjustment[compressed & ~unit::SIGN_MASK];
        vec3 vec(
                uvadj * (float32)xbits,
                uvadj * (float32)ybits,
                uvadj * (float32)(126 - xbits - ybits));

        // set all the sign bits
        if (compressed & unit::XSIGN_MASK)
        {
            X(vec) = -X(vec);
        }
        if (compressed & unit::YSIGN_MASK)
        {
            Y(vec) = -Y(vec);
        }
        if (compressed & unit::ZSIGN_MASK)
        {
            Z(vec) = -Z(vec);
        }
    ...
    const Angle
    AngleBetween(const vec3& a, const vec3& b, const vec3& perpa)
    {
        const Angle s = AngleBetween(a, b);
        const Angle t = AngleBetween(perpa, b);
        if (t.inDegrees() > PWN_MATH_VALUE(90.0))
        {
            return Angle::FromDegrees(180) + s;
        }
        else
        {
            return s;
