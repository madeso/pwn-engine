...
namespace render
{
    namespace fse
    {
        Link::Link(const string& aname) : name(aname)
        {
        }
        ...
        void
        Link::provide(World3* world, RenderArgs ra)
        {
            prov->doProvide(world, ra);
        }

        void
        Link::sortout(LinkerPtr usr)
        {
            prov = usr->getProvider(name);
        }

        void
        Provider::setId(const string& aid)
        {
            if (id.empty())
            {
                id = aid;
            ...
            {
                throw FseException(
                        "Unable to change id from " + id + " to " + aid);
            }
        }

        const string
        Provider::getId() const
        {
            return id;
        }

        void
        Provider::provide(World3* world, const RenderArgs& ra)
        {
            if (autocallCommands)
            {
                callCommands();
        ...
        Provider::link(Linker* linker)
        {
            if (false == targetname.empty())
            {
                ...
                {
                    e.addMessage(
                            core::Str() << "From provider: " << toString());
                    throw;
                }
        ...
        void
        Provider::bind(Binder* bd, ShaderPool* pool)
        {
            const string file = shaderdata.data();
            if (file.empty())
            {
                boost::optional<const core::Ptree&> source =
                        shaderdata.get_child_optional("source");

                if (source)
                {
                    shader = pool->getFromSource(
                            source.get(),
                            core::Str() << this->file << ":" << id);
                }
            }
            else
            {
        ...
        BufferReferencePtr
        Provider::createBuffer(const string& name)
        {
            return BufferReferencePtr(new BufferReference(name));
        }

        void
        Provider::postlink(Linker* linker)
        {
            BOOST_FOREACH (CommandPtr c, commands)
            {
                c->link(linker);
        ...
        void
        Provider::denyAutocallOfCommands()
        {
            autocallCommands = false;
        }

        void
        Provider::callCommands()
        {
            BOOST_FOREACH (CommandPtr c, commands)
            {
                c->apply();
            ...
            , file(afile)
        {
            ...
            BOOST_FOREACH (
                    const core::Ptree::value_type& e,
                    el.get_child("commands"))
            {
        ...
        string
        Provider::toString() const
        {
            return core::Str() << getId() << "(" << targetname << "): <"
