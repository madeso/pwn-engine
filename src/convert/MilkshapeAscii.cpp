    ...
    namespace milkshape
    {
        namespace ascii
        {
            ...
            public:
                ...
                void
                readBones(const std::vector<string>& cmd)
                {
                ...
                void
                readMaterials(const std::vector<string>& cmd)
                {
                ...
                void
                readMeshes(const std::vector<string>& cmd)
                {
                    // number of meshes
                ...
                void
                readSingleNormal(Mesh& mesh)
                {
                    string normalline = readLine();
                    std::vector<string> normalcmd =
                            SplitString(normalline, " ");
                ...
                void
                readSingleVertex(Mesh& mesh)
                {
                    string vertexline = readLine();
                    std::vector<string> vertexcmd =
                            SplitString(vertexline, " ");
                ...
                void
                readCurrentFrame(std::vector<string> cmd)
                {
                    model.currentFrame =
                            static_cast<pwn::real>(intParse(cmd[1]));
                }

                void
                readFrameCount(std::vector<string> cmd)
                {
            ...
            void
            Read(BuilderList* builders, const pwn::string& path)
            {
                ...
                Entry e(builder);
                e.animations.push_back(
                        MilkshapeCommon::ExtractAnimation(runner.model));
