...
namespace render
{
    namespace fse
    {
        namespace
        {
            ...
            {
                CreateFboFunc(int w, int h) : width(w), height(h)
                {
                }
            ...
            {
                GetFromPoolFunc(core::Pool<T>* p) : pool(p)
                {
                }
        ...
        FboPtr
        FboPool::create(const string& name)
        {
            return map.get(name);
        }

        void
        FboPool::release(FboPtr fbo)
        {
            pool.release(fbo);
        }
        ...
        ShaderPtr
        Binder::getShader(const string& shadername)
        {
            return shaders.get(shadername);
        }

        ShaderPtr
        Binder::getShaderOrNull(const string& shadername)
        {
            if (shadername.empty())
            {
                return ShaderPtr();
        ...
        void
        Binder::reference(BufferReferencePtr br)
        {
            if (references.find(br) != references.end())
            {
                throw FseException(
                        br->getName() + " already addd to " + toString());
            }
        ...
        void
        Binder::associate(const string& p, Size size)
        {
            associations[p] = size;
        }

        FboPtr
        Binder::allocate(const string& name)
        {
            return getCreator(name)->create(name);
        }
        FboCreatorPtr
        Binder::getCreator(const string& name)
        {
            return getCreator(sizeOf(name));
        }

        FboCreatorPtr
        Binder::getCreator(Size size)
        {
        ...
        void
        Binder::release(FboPtr buff)
        {
            getCreator(Size(buff->getWidth(), buff->getHeight()))
                    ->release(buff);
        }

        string
        Binder::toString() const
        {
            return "";
        }

        Size
        Binder::sizeOf(const string& name) const
        {
