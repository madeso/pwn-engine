...
namespace convert
{
    namespace an8
    {
        struct Term
        {
            ...
            Term() : type(Unknown), line(0)
            {
            }

            Term(Type t, unsigned int l) : type(t), line(l)
            {
            }
        ...
        Str&
        operator<<(Str& str, const Term& t)
        {
            str << "(";

            switch (t.type)
            {
            case Term::Number: str << "NUMBER " << t.value; break;
            case Term::String: str << "STRING " << t.value; break;
            case Term::Ident: str << " " << t.value; break;
            case Term::Begin: str << "BEGIN"; break;
            case Term::End: str << "END"; break;
            }
        ...
        public:
            ...
            Lexer() : state(sNone), line(1), ignoreChar(0), inComment(false)
            {
            }
            ...
            on(char c)
            {
                ...
                default:
                    Throw(Str()
                          << "(" << line
                          << ") Internal error: unknown lexer state " << state);
                }
            }
        ...
        public:
            ...
            bool
            peek(Term* term, unsigned int asteps = 1) const
            {
        ...
        public:
            ...
            explicit Child(const pwn::string& n = "") : name(n)
            {
            }

            pwn::string
            getString(size_t i = 0) const
            {
                return values[i];
            }
            pwn::real
            getNumber(size_t i = 0) const
            {
                return boost::lexical_cast<pwn::real>(getString(i));
            }
            int
            getInt(size_t i = 0) const
            {
            ...
            void
            addValue(const std::string& val)
            {
                values.push_back(val);
            }

            void
            addChild(ChildPtr child)
            {
                const pwn::string name = child->getName();
                ChildMap::iterator r = childs.find(name);

                ChildList& list = (r != childs.end())
                        ? r->second  // if we found one, get a reference to the list
                        : childs.insert(ChildMap::value_type(name, ChildList()))
                                  .first
                                  ->second;  // if not create one and get the reference
                list.push_back(child);
            }

            ChildPtr
            getChild(const pwn::string& name)
            {
                ChildMap::iterator r = childs.find(name);
                if (r == childs.end())
                {
                ...
                {
                    Throw(Str()
                          << "no unique child when searching for " << name);
                }
                return r->second[0];
            }

            const ChildList&
            getChilds(const pwn::string& name) const
            {
            ...
            ChildList&
            getChilds(const pwn::string& name)
            {
                ChildMap::iterator r = childs.find(name);
                if (r == childs.end())
                {
            ...
            bool
            hasChild(const pwn::string& name) const
            {
                ChildMap::const_iterator r = childs.find(name);
                return r != childs.end();
            }
            ...
            ChildPtr
            getChild(const std::vector<pwn::string>& names)
            {
        ...
        void
        ParseChildren(ChildPtr child, TermReader* tr)
        {
            Term t;
            ...
            while (tr->peek(&t) && t.type != Term::End)
            {
                ...
                case Term::Ident: {
                    ...
                    else
                    {
                        ...
                        {
                            Throw(Str()
                                  << "Syntax error, expecting a BEGIN got "
                                  << start);
                        }
                        ParseChildren(c, tr);
                        ...
                        {
                            Throw(Str() << "Syntax error, expecting a END got "
                                        << end);
                        }
                        child->addChild(c);
                    }
                ...
                case Term::Number:
                    ...
                    {
                        Throw(Str()
                              << "Syntax error, not expecting a Number, got "
                              << tr);
                    }
                    child->addValue(t.value);
                    break;
                case Term::String:
                    ...
                    {
                        Throw(Str()
                              << "Syntax error, not expecting a String, got "
                              << tr);
                    }
                    child->addValue(t.value);
                    break;
                case Term::Begin:
                    Throw(Str()
                          << "Syntax error, not expecting a BEGIN, got " << tr);
                    break;
                }
            }
        }
        ...
        std::vector<pwn::math::vec3>
        ExtractPoints(ChildPtr points)
        {
            std::vector<pwn::math::vec3> r;
            const size_t count = points->getValueCount();
            for (size_t i = 0; i < count; i += 3)
            {
                r.push_back(pwn::math::vec3(
                        points->getNumber(i),
                        points->getNumber(i + 1),
                        points->getNumber(i + 2)));
            }
            return r;
        }
        std::vector<pwn::math::vec2>
        ExtractTextcoords(ChildPtr points)
        {
            std::vector<pwn::math::vec2> r;
            const size_t count = points->getValueCount();
            for (size_t i = 0; i < count; i += 2)
            {
                r.push_back(pwn::math::vec2(
                        points->getNumber(i),
                        points->getNumber(i + 1)));
            }
            return r;
        ...
        ExtractMaterials(ChildPtr materials)
        {
        ...
        struct Color
        {
            ...
            Color(const math::Rgba& r) : rgb(r.r, r.g, r.b)
            {
            }
            pwn::math::Rgb rgb;
            pwn::string texture;
        ...
        ExtractMaterial(ChildPtr ma)
        {
            ...
            m.specular = s->hasChild("specular")
                    ? ExtractColor(s->getChild("specular"))
                    : d.specular;
            m.alpha = s->hasChild("alpha")
                    ? s->getChild("alpha")->getNumber(0) / 255.0f
                    : 1.0f;
            m.emissive = s->hasChild("emissive")
                    ? ExtractColor(s->getChild("emissive"))
                    : d.emission;
        ...
        void
        ExtractMeshOnObject(Object& r, ChildPtr o, const pwn::string& type)
        {
            if (o->hasChild(type))
            {
        ...
        ExtractObject(ChildPtr o)
        {
            ...
            BOOST_FOREACH (ChildPtr m, o->getChilds("material"))
            {
                ...
                r.materials.insert(
                        StringMaterialMap::value_type(mat.name, mat));
            }
            return r;
        }
        ...
        math::mat44
        ExtractBase(ChildPtr owner, const pwn::string& name)
        {
            ...
            const math::quat rotation =
                    ExtractQuaternion(c->getChild("rotation"));
            return math::mat44helper(math::mat44Identity())
                    .rotate(rotation)
                    .translate(-origin)
                    .mat;
        }
        ...
        struct Bone
        {
            Bone() : length(0), rotation(math::qIdentity())
            {
            }
            math::vec3
            vec() const
            ...
            const Bone*
            getBone(const pwn::string& s) const
            {
                if (name == s)
                {
                    return this;
        ...
        struct Figure
        {
            ...
            Bone
            getBone(const pwn::string& name) const
            {
        ...
        struct File
        {
            ...
            Object
            getObject(const pwn::string objectName) const
            {
            ...
            Figure
            getFigure(const pwn::string objectName) const
            {
        ...
        std::map<pwn::string, pwn::string>
        ExtractTextures(ChildPtr file)
        {
        ...
        struct BoneIndexes
        {
            ...
            void
            add(const Bone* b, mesh::BoneIndex i)
            {
                map.insert(BoneIndexMap::value_type(b, i));
            }

            mesh::BoneIndex
            get(const Bone* b) const
            {
                BoneIndexMap::const_iterator r = map.find(b);
                if (r == map.end())
                {
                    Throw(Str()
                          << "Bone " << b->name << " has not been registred");
                }
                return r->second;
            }
        };
        ...
        struct BoneAssignmentBuilder
        {
            ...
            {
                bool
                operator()(const Ba& lhs, const Ba& rhs) const
                {
            ...
            BaSet bas;
            void
            add(mesh::BoneIndex i, real v)
            {
                bas.insert(Ba(i, v));
            }
            math::vec4
            getAssignment(mesh::BoneIndex bi) const
            {
                if (bas.empty())
                {
        ...
        public:
            ...
            math::vec3
            getPoint(real t) const
            {
                return start + dir * t;
            }

            real
            project(const math::vec3& p) const
            {
            ...
            bool
            getDistance(const math::vec3& p, real* d, real* os) const
            {
        ...
        public:
            ...
            const real
            getRadiusSquared() const
            {
                return radiuss;
            }

            real
            getDistanceToCenterSquared(const math::vec3& p) const
            {
        ...
        {
        public:
            Capsule(const math::vec3& astart,
                    const math::vec3& aend,
                    real r1,
                    real r2)
                : start(astart, r1)
                , middle(astart, aend)
            ...
            bool
            contains(const math::vec3& p, real* dist, real* range) const
            {
                real scale = 0;
                real d = 0;
                ...
                {
                    *range = math::Lerp(
                            start.getRadiusSquared(),
                            scale,
                            end.getRadiusSquared());
                    *dist = d;
                }
                else if (scale >= 1)
                {
        ...
        real
        GetPossibleAssignment(const Bone* b, const math::vec3& xyz)
        {
            // todo
            const Influence& i = b->influence;
            math::mat44helper h(b->xform);
            Ray r(h.transform(math::vec3(0, 0, 0)),
                  h.transform(math::vec3(0, 1, 0)));
        ...
        math::vec4
        GetBoneAssignment(BoneAssignment* a, const math::vec3& xyz)
        {
        ...
        public:
                    ...
                    BoneAssignment* ba)
            {
                ...
                BOOST_FOREACH (const Mesh& m, o.meshes)
                {
                    ...
                    BOOST_FOREACH (const Face& f, m.faces)
                    {
                        ...
                        {
                            verts.push_back(pwn::mesh::BTriangle::Vertex(
                                    pointBase + p.point,
                                    0,
                                    textureBase + p.texture));
                        }
                        std::vector<pwn::mesh::BTriangle::Vertex> toadd(
                                verts.rbegin(),
                                verts.rend());
                        b->addFace(
                                getOrAddMaterial(o, b, m.materials[f.material]),
                                toadd);
                    }
            ...
            void
            addToBuilder(const Figure& o, pwn::mesh::Builder* builder)
            {
                BoneIndexes bi;
            ...
            void
            prepareBones(Bone& b, const math::mat44& root)
            {
                const math::mat44 rotatedr =
                        math::mat44helper(root).rotate(b.rotation).mat;
                BOOST_FOREACH (Bone& c, b.childs)
                {
                    prepareBones(
                            c,
                            math::mat44helper(rotatedr).translate(b.vec()).mat);
                }
                b.xform = rotatedr;
            }
        ...
        void
        ExtractObjectNames(const File& f, std::set<pwn::string>* s)
        {
        ...
        void
        RemoveObjectNames(const Bone& b, std::set<pwn::string>* s)
        {
            BOOST_FOREACH (const NamedObject& n, b.objects)
            {
                std::set<pwn::string>::const_iterator r =
                        s->find(kObjectId + n.object);
                if (r != s->end())
                {
                    s->erase(r);
                }
        ...
        void
        ExtractFigureNames(const File& f, std::set<pwn::string>* s)
        {
        ...
        struct An8KeyFrame
        {
            An8KeyFrame(int f, real av) : frame(f), v(av)
            {
            }
            int frame;
            real v;
        ...
        std::size_t
        GetBoneIndex(const mesh::Builder& b, const pwn::string name)
        {
                ...
                const Sequence& s)
        {
            ...
            for (int bone = 0; bone < numbones; ++bone)
            {
                ...
                {
                    if (cdata[0][bone].has(frame) ||
                        cdata[1][bone].has(frame) || cdata[2][bone].has(frame))
                    {
                        const real x = cdata[0][bone].get(frame);
                        const real y = cdata[1][bone].get(frame);
                        const real z = cdata[2][bone].get(frame);
                        afp.push_back(
                                mesh::FramePosition(frame, b.bones[bone].pos));
                        afr.push_back(mesh::FrameRotation(
                                frame,
                                math::cquat(x, y, z)));
                    }
                }
