    ...
    namespace
    {
        ...
        public:
            ImmediateMode(
                    ...
                    const mesh::Mesh::TriList& tridata)
                : smesh(smesh)
                , triangles(tridata)
            {
            }
            ...
            void
            render(const mesh::CompiledPose& pose)
            {
                pwnAssert_NoGLError();
                    ...
                    for (int i = 0; i < 3; ++i)
                    {
                        ...
                        else
                        {
                            ...
                            for (int b = 0; b < 4; ++b)
                            {
                                ...
                                const mesh::BoneIndex boneIndex =
                                        mesh::GetBoneIndex(val);
                                infsum += inf;
                                normal += math::GetNormalized(
                                        math::cmat33(math::SetTransform(
                                                pose.transforms[boneIndex]
                                                        .first,
                                                math::vec3(0, 0, 0))) *
                                        point.normal);
                                vertex += math::cvec3(
                                        pose.transforms[boneIndex].first *
                                        math::cvec4(point.position));
                            }
                            Assert(infsum > 0);
                            const real inv = 1 / infsum;
                            vertex *= inv;
                            normal = math::GetNormalized(
                                    math::vec3(inv * normal));
                        }
        ...
        public:
            ...
            CompiledMesh*
            compile(const mesh::Mesh::TriList& tridata)
            {
