...
namespace pwn
{
	namespace math
	{
		...
		{
			for(int i = 0; i < 3 * 3; ++i) if(!equal(lhs.data()[i], rhs.data()[i]))
				{
					return false;
				}
			return true;
		...
		{
			for(int i = 0; i < 4 * 4; ++i) if(!equal(lhs.data()[i], rhs.data()[i]))
				{
					return false;
				}
			return true;
		...
		{
			if(equal(lhs.angle.inDegrees(), rhs.angle.inDegrees()) && equal(lhs.angle.inDegrees(), 0))
			{
