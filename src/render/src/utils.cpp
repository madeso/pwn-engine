...
{
    void
    SetView2d(int width, int height)
    {
    ...
    void
    RenderFullscreenQuad(int unknown, int width, int height)
    {
            ...
            real size)
    {
        ...
        for (int i = -halfCount; i < halfCount; ++i)
        {
            ...
            lines->add(
                    math::Left() * -size + math::In() * d,
                    math::Left() * size + math::In() * d);
            lines->add(
                    math::In() * -size + math::Left() * d,
                    math::In() * size + math::Left() * d);
        }
...
string
Nullstring(const string& str, const string& def)
{
    if (str.empty() == false)
    {
        return str;
