...
namespace math
{
    ...
    const quat
    cquat(const real ax, const real ay, const real az)
    {
        quat q;
    ...
    const quat
    quatw(const real aw, const real ax, const real ay, const real az)
    {
        return quat(ax, ay, az, aw);
    }

    const quat
    Lerp(const quat& f, const real scale, const quat& t)
    {
        return cml::lerp(f, t, scale);
    }

    const quat
    Qlerp(const quat& f, const real scale, const quat& t)
    {
    ...
    const quat
    Slerp(const quat& f, const real scale, const quat& t)
    {
    ...
    const quat
    SlerpShortway(const quat& f, const real scale, const quat& t)
    {
    ...
    const vec3
    RotateAroundOrigoNorm(const quat& q, const vec3& v)
    {
    ...
    const vec3
    RightUpIn(const quat& q, const vec3& v)
    {
        return Right(q) * X(v) + Up(q) * Y(v) + In(q) * Z(v);
    }

    const vec3
    RotateAroundOrigo(const quat& q, const vec3& v)
    {
    ...
    const quat
    Combine_Local(const quat& current, const quat& extra)
    {
        return GetNormalized(current * extra);
    }

    const quat
    Combine_Parent(const quat& current, const quat& extra)
    {
        return GetNormalized(extra * current);
    }

    const quat
    Combine(const quat& current, const quat& extra)
    {
    ...
    qIdentity()
    {
        return quat(
                PWN_MATH_VALUE(1.0),
                vec3(PWN_MATH_VALUE(0.0),
                     PWN_MATH_VALUE(0.0),
                     PWN_MATH_VALUE(0.0)));
    }

    const quat
    qLookAt(const vec3& from, const vec3& to, const vec3& up)
    {
        return qLookInDirection(to - from, up);
    }

    const quat
    qLookAtOrNot(const vec3& from, const vec3& to, const vec3& up)
    {
    ...
    const quat
    qLookInDirection(const vec3& adir, const vec3& up)
    {
        quat ret;
        mat44 mat;
