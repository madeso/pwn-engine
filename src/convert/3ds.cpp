        ...
        namespace hidden
        {
            namespace
            {
                ...
                public:
                    ...
                    void
                    read(byte* buffer, std::size_t size)
                    {
                ...
                struct BinaryChunk
                {
                    ...
                    static std::vector<BinaryChunkPtr>
                    Parse(byte* bytes, const std::size_t size)
                    {
                    ...
                    static BinaryChunkPtr
                    SelectChunk(int id, std::vector<BinaryChunkPtr>& s)
                    {
                    ...
                    static std::vector<BinaryChunkPtr>
                    IterateChunks(int id, std::vector<BinaryChunkPtr> s)
                    {
                        std::vector<BinaryChunkPtr> res;
                    ...
                    static BinaryChunkPtr
                    SelectChunkOrNull(int id, std::vector<BinaryChunkPtr>& s)
                    {
                        BOOST_FOREACH (BinaryChunkPtr& c, s)
                        {
                            if (c->id == id)
                            ...
                            Binary& b)
                    {
                        while (b.Continue())
                        {
                            ...
                            chunks.push_back(BinaryChunkPtr(new BinaryChunk(
                                    id,
                                    b.getBytes(size),
                                    size)));
                        }
                ...
                {
                    LocalCoordinateSystemChunk() : mat(mat44Identity())
                    {
                    }
                    ...
                    void
                    load(BinaryChunkPtr c)
                    {
                        mat = GetMatrix(c);
                    }

                    vec3
                    translate(vec3 p) const
                    {
                        return mat * p;
                    }
                };
                ...
                struct TriMeshChunk
                {
                    struct Poly
                    {
                        Poly(int aa, int bb, int cc) : a(aa), b(bb), c(cc)
                        {
                        }
                    ...
                    load(BinaryChunkPtr c)
                    {
                        ...
                        points = ParsePoints(BinaryChunk::SelectChunk(
                                ChunkId::VERTICES_LIST,
                                chunks));
                        mapping = ParseMapping(BinaryChunk::SelectChunk(
                                ChunkId::MAPPING_COORDINATES_LIST,
                                chunks));
                        std::vector<BinaryChunkPtr> facechunks;
                        faces = ParseFaces(
                                BinaryChunk::SelectChunk(
                                        ChunkId::FACES_DESCRIPTION,
                                        chunks),
                                facechunks);
                        BOOST_FOREACH (
                                const BinaryChunkPtr& bc,
                                BinaryChunk::IterateChunks(
                                        ChunkId::FACES_MATERIAL,
                                        facechunks))
                        {
                            facematerials.push_back(
                                    FaceMaterialChunk().load(bc));
                        }
                        coordsys.load(BinaryChunk::SelectChunkOrNull(
                                ChunkId::LOCAL_COORDINATES_SYSTEM,
                                chunks));
                    }
                ...
                struct ObjectChunk
                {
                    ...
                    load(BinaryChunkPtr c)
                    {
                        ...
                        name = b.ReadString();
                        BinaryChunk::ReadFromBinary(chunks, b);
                        BinaryChunkPtr tm = BinaryChunk::SelectChunk(
                                ChunkId::TRIANGULAR_MESH,
                                chunks);
                        if (tm.get() == 0)
                        {
                ...
                struct MaterialChunk
                {
                    ...
                    load(BinaryChunkPtr c)
                    {
                        ...
                        name.load(BinaryChunk::SelectChunk(
                                ChunkId::MATERIAL_NAME,
                                chunks));
                        ambient.load(BinaryChunk::SelectChunkOrNull(
                                ChunkId::AMBIENT_COLOR,
                                chunks));
                        diffuse.load(BinaryChunk::SelectChunkOrNull(
                                ChunkId::DIFFUSE_COLOR,
                                chunks));
                        specular.load(BinaryChunk::SelectChunkOrNull(
                                ChunkId::SPECULAR_COLOR,
                                chunks));
                        texture.load(
                                BinaryChunk::SelectChunk(
                                        ChunkId::TEXTURE_MAP_1,
                                        chunks)
                                        ->getSub(ChunkId::MAPPING_FILENAME));
                ...
                struct Editor3dChunk
                {
                    ...
                    load(BinaryChunkPtr c)
                    {
                        ...
                        BOOST_FOREACH (
                                const BinaryChunkPtr& oc,
                                BinaryChunk::IterateChunks(
                                        ChunkId::OBJECT_BLOCK,
                                        chunks))
                        {
                            objects.push_back(ObjectChunk().load(oc));
                        }
                        BOOST_FOREACH (
                                const BinaryChunkPtr& mc,
                                BinaryChunk::IterateChunks(
                                        ChunkId::MATERIAL_BLOCK,
                                        chunks))
                        {
                ...
                static void
                ParseChunk(pwn::mesh::Builder* builder, MainChunk& main)
                {
                    ...
                    {
                        pwn::mesh::BTriangle::index ibase =
                                builder->positions.size();
                        for (std::size_t i = 0; i < chunk.trimesh.points.size();
                             ++i)
                        {
                            builder->addPosition(pwn::mesh::BPoint(
                                    chunk.trimesh.coordsys.translate(
                                            chunk.trimesh.points[i]),
                                    0));
                            builder->addTextCoord(chunk.trimesh.mapping[i]);
                        }
                        BOOST_FOREACH (
                                FaceMaterialChunk fmc,
                                chunk.trimesh.facematerials)
                        {
                            pwn::uint32 material =
                                    builder->getMaterial(fmc.name);
                            BOOST_FOREACH (int faceindex, fmc.faces)
                            {
                                const TriMeshChunk::Poly p =
                                        chunk.trimesh.faces[faceindex];

                                const pwn::mesh::BTriangle::index a =
                                        ibase + p.a;
                                const pwn::mesh::BTriangle::index b =
                                        ibase + p.b;
                                const pwn::mesh::BTriangle::index c =
                                        ibase + p.c;

                                builder->addTriangle(
                                        material,
                                        pwn::mesh::BTriangle(
                                                pwn::mesh::BTriangle::Vertex(
                                                        a,
                                                        0,
                                                        a),
                                                pwn::mesh::BTriangle::Vertex(
                ...
                void
                Load(pwn::mesh::Builder* builder, const string& filename)
                {
                    std::vector<BinaryChunkPtr> chunks;
