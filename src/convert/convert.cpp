...
namespace convert
{
    Entry::Entry(const mesh::Builder& b, const string& n) : builder(b), name(n)
    {
    }
    ...
    const InputFormat*
    SuggestFormatData(const pwn::string& name)
    {
         ...
         bool verbose)
    {
        ...
        if (fileFormat)
        {
            if (verbose)
            {
                cout << "reading " << inputfile << " (" << fileFormat->getName()
                     << ").." << std::endl;
            }
        ...
        {
            std::cerr << "Unable to determine the kind of reader to use with "
                      << inputfile;
            return false;
        }
    }
    ...
    bool
    SuggestAnimationFile(const pwn::string& in, pwn::string* out)
    {
    ...
    mesh::Animation
    ExtractDefaultAnimation(const mesh::Builder& builder)
    {
    ...
    struct ConvertData
    {
        ...
        float modelScale;
        pwn::string
                texturedir;  // relative directory when loading textures from game
        pwn::string animdir;  // if empty a default based on outdir is created
        pwn::string
                outdir;  // if emppy a default based on input file is created
        bool runStatistics;
        bool verbose;
        bool meshInfo;
        bool writeResult;
    ...
    pwn::string
    CombineFilename(const pwn::string& name, const pwn::string& object)
    {
        if (object.empty())
        {
            return name;
    ...
    AnimationExtract
    LoadAnimations(const pwn::string& filename)
    {
    ...
    bool
    Convert(const pwn::string& argv0,
            const ConvertData& cd,
            const std::vector<pwn::string>& subobjects,
            const pwn::string& ainputfile,
            const InputFormat* formatOveride)
    {
        ...
        if (Load(&builders, subobjects, inputfile, formatOveride, cd.verbose) ==
            false)
        {
            return false;
        }
        ...
        BOOST_FOREACH (Entry& e, builders)
        {
            ...
            pwn::string texturedir = cd.texturedir == "@"
                    ? SuggestTextureDirectory(inputfile)
                    : cd.texturedir;
            pwn::mesh::MoveTextures(&builder, texturedir);

            if (cd.writeDefaultAnimation && e.builder.bones.empty() == false)
            {
                e.animations.push_back(AnimationEntry(
                        ExtractDefaultAnimation(e.builder),
                        "_default"));
            }
            ...
            {
                cerr << inputfile << " failed validation with "
                     << validationErrors << " error(s)... ignoring file..."
                     << endl;
                return false;
            }
            ...
            if (cd.meshInfo)
            {
                cout << "Mesh information " << name << ":" << endl
            ...
            if (cd.writeResult)
            {
                ...
                if (SuggestAnimationFile(inputfile, &animationfile))
                {
                    ...
                    BOOST_FOREACH (AnimationEntry& ae, e.animations)
                    {
                        ...
                        {
                            BOOST_FOREACH (
                                    const pwn::mesh::AnimationInformation& ai,
                                    animationsToExtract)
                            {
            ...
            const pwn::string& val)
    {
        ...
        std::cout << "Usage: " << usage << std::endl
                  ...
                  << "Description: " << args->displayDescription(val)
                  << std::endl;
    ...
    int
    Command_RunStatistics(App* app, core::ConsoleArguments<App>* args)
    {
    ...
    int
    Command_MeshInfo(App* app, core::ConsoleArguments<App>* args)
    {
    ...
    int
    Command_DontWriteResult(App* app, core::ConsoleArguments<App>* args)
    {
    ...
    int
    Command_Verbose(App* app, core::ConsoleArguments<App>* args)
    {
        app->arg.verbose = true;
        return 0;
    }
    int
    Command_Restore(App* app, core::ConsoleArguments<App>* args)
    {
        app->arg = app->old;
        return 0;
    }
            ...
            const pwn::string& val)
    {
        ...
        {
            std::cerr << "Unknown format override: " << val
                      << " using last override";
            return 1;
        }
    }
    ...
    int
    Command_DontStopOnErrors(App* app, core::ConsoleArguments<App>* args)
    {
    ...
    int
    Command_DisplayArguments(App* app, core::ConsoleArguments<App>* args)
    {
    ...
    App::handle(int argc, char* argv[])
    {
        ...
        args.setArgCommand(
                Strings() << "scale"
                          << "s",
                CommandArg_Scale,
                "scales the loaded mesh");
        args.setArgCommand(
                Strings() << "texturedir"
                          << "tex"
                          << "t",
                CommandArg_TextureDir,
                "sets the texture dir that will be used when loading, @ means meshfilename");
        args.setArgCommand(
                Strings() << "animdir"
                          << "anim"
                          << "a",
                CommandArg_AnimDir,
                "sets the animation dir");
        args.setArgCommand(
                Strings() << "outdir"
                          << "out"
                          << "o",
                CommandArg_OutDir,
                "sets the outdir where to place the result, defaults to meshdir");
        args.setArgCommand(
                Strings() << "format"
                          << "force"
                          << "f",
        ...
        args.setArgCommand(
                Strings() << "run-file"
                          ...
                          << "F",
                CommandArg_RunFile,
                "forces a file to run, needed if the path starts with / or -");
        args.setArgCommand(
                Strings() << "object"
                          << "obj"
                          << "O",
                CommandArg_AddSubObject,
                "adds a subobject to be converted, if not set all will be converted");
        ...
        args.setArgCommand(
                Strings() << "default-anim"
                          << "def"
                          << "d",
                CommandArg_WriteDefaultAnim,
                "If Y or y, then the default animation is written (default y)");

        args.setCommand(
                Strings() << "showstat"
                          << "stat"
                          << "S",
                Command_RunStatistics,
                "Show statistics for the mesh");
        args.setCommand(
                Strings() << "info"
                          << "i",
                Command_MeshInfo,
                "Show mesh informatio");
        args.setCommand(
                Strings() << "nowrite"
                          << "w",
                Command_DontWriteResult,
                "Only run logic, dont write the result");
        args.setCommand(
                Strings() << "verbose"
                          << "v",
                Command_Verbose,
        ...
        args.setCommand(
                Strings() << "keep"
                          << "k",
                Command_Restore,
                "Keep the options for the last mesh");
        args.setCommand(
                Strings() << "no-break-on-error"
                          << "no-break"
                          << "B",
                Command_DontStopOnErrors,
                "Forces convertion to run even after errors");
        args.setCommand(
                Strings() << "help"
                          << "h",
                Command_DisplayArguments,
