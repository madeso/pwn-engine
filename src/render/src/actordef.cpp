...
namespace render
{
    ...
    boost::shared_ptr<render::Material>
    Compile(const mesh::Material mm, TexturePool2* pool)
    {
    ...
    Compile(const mesh::Mesh& mesh, TexturePool2* pool)
    {
        boost::shared_ptr<ActorDef> def(new ActorDef());
        std::auto_ptr<MeshCompiler> compiler(
                CreateMeshCompiler_ImmediateMode(mesh));

        // todo: implement better rendering
        BOOST_FOREACH (
                mesh::Mesh::TriangleMap::const_reference r,
                mesh.getTriangles())
        {
