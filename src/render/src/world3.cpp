...
#pragma warning(                                                               \
        disable : 4512)  // warning C4512: 'boost::detail::addr_impl_ref<T>' : assignment operator could not be generated
...
namespace render
{
    ...
    {
    public:
        BasicWorld() : list(true), ambient(0.2f, 0.2f, 0.2f, 1.0f)
        {
        }

        void
        actor_add(ActorPtr actor)
        {
            actors.push_back(actor);
        }

        void
        actor_remove(ActorPtr actor)
        {
            ActorList::iterator res =
        ...
        void
        lines_add(LinesPtr line)
        {
            lines.push_back(line);
        }

        void
        lines_remove(LinesPtr line)
        {
            LinesList::iterator res =
        ...
        void
        light_add(LightPtr light)
        {
            lights.push_back(light);
        }

        void
        light_remove(LightPtr light)
        {
            LightList::iterator res =
                    std::find(lights.begin(), lights.end(), light);
            if (res == lights.end())
            {
                return;
            }
        ...
        void
        light_setAmbient(const math::Rgba& c)
        {
            ambient = c;
        }

        void
        render(const RenderArgs& r) const
        {
            ...
            list.end(
                    lights.empty() ==
                    false);  // apply materials if there are lights
        ...
        mutable RenderList
                list;  // mutable since we want to use frame-to-frame coherence, and avoid uneccessary reallocation of its internals
