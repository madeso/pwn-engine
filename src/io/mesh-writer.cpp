...
namespace io
{
    ...
    namespace
    {
        ...
        {
            static mesh::Mesh::TriList&
            GetTriList(mesh::Mesh::TriangleMap& map, pwn::uint32 id)
            {
                return map[id];
            }
        };
        ...
        {
            static const mesh::Mesh::TriList&
            GetTriList(const mesh::Mesh::TriangleMap& map, pwn::uint32 id)
            {
    ...
    public:
        template <class Filer>
        static void
        handle(Filer& vf, MeshArg mesh, VersionType version)
        {
            vf.handle8(version);
            if (version != kVersion)
            {
            ...
            {
                vf.handleVector(
                        MeshUtil<Filer>::GetTriList(mesh.triangles, id));
            }
    ...
    void
    Write(const mesh::Mesh& mesh, const pwn::string& filename)
    {
        Assert(mesh.validate(true) == 0);
        VirtualFile vf(filename, false);
        FileWriter w;
        w.file = &vf;
        MeshFile<const mesh::Mesh&, const pwn::uint8>::handle(
                w,
                mesh,
                kVersion);
    }
    ...
    void
    Read(mesh::Mesh* mesh, const pwn::string& filename)
    {
