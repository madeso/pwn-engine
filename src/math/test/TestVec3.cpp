...
GTEST(testNormal)
{
    EXPECT_PRED_FORMAT2(
            ::pwn::math::vec3_equal_test,
            vec3(1, 0, 0),
            GetNormalized(vec3(4, 0, 0)));
    EXPECT_PRED_FORMAT2(
            ::pwn::math::vec3_equal_test,
            vec3(0, 1, 0),
            GetNormalized(vec3(0, 9, 0)));
}
...
GTEST(testAngle)
{
    ...
    EXPECT_REAL_EQ(
            180,
            AngleBetween(vec3(1, 0, 0), vec3(-1, 0, 0)).inDegrees());
    EXPECT_REAL_EQ(90, AngleBetween(vec3(1, 0, 0), vec3(0, -1, 0)).inDegrees());

    // specify extra - 0-360
    EXPECT_REAL_EQ(
            0,
            AngleBetween(vec3(1, 0, 0), vec3(1, 0, 0), vec3(0, 1, 0))
                    .inDegrees());
    EXPECT_REAL_EQ(
            90,
            AngleBetween(vec3(1, 0, 0), vec3(0, 1, 0), vec3(0, 1, 0))
                    .inDegrees());
    EXPECT_REAL_EQ(
            180,
            AngleBetween(vec3(1, 0, 0), vec3(-1, 0, 0), vec3(0, 1, 0))
...
GTEST(testLerp)
{
    EXPECT_PRED_FORMAT2(
            ::pwn::math::vec3_equal_test,
            vec3(0, 1, 0),
            Lerp(vec3(0, 1, 0), 0, vec3(2, 4, 6)));
    EXPECT_PRED_FORMAT2(
            ::pwn::math::vec3_equal_test,
            vec3(2, 4, 6),
            Lerp(vec3(0, 1, 0), 1, vec3(2, 4, 6)));
    EXPECT_PRED_FORMAT2(
            ::pwn::math::vec3_equal_test,
            vec3(3, 5, 7),
            Lerp(vec3(2, 4, 6), 0.5f, vec3(4, 6, 8)));
}
}
