...
namespace engine
{
    ...
    {
        void
        UpdateSystem(GameImp::SystemPtr& system)
        {
            system->update();
        }
    }
    ...
    void
    GameImp::updateSystems()
    {
        std::for_each(systems.begin(), systems.end(), UpdateSystem);
    }

    void
    GameImp::display_add(int id, Display* disp)
    {
    ...
    void
    GameImp::display(int id, render::VirtualDisplay& world)
    {
        const DisplayMap::iterator res = displays.find(id);
        if (res == displays.end())
        {
    ...
    void
    GameImp::display_remove(int id, Display* disp)
    {
        const DisplayMap::iterator res = displays.find(id);
        if (res == displays.end())
        {
    ...
    void
    GameImp::handleKey(Key::Code key, bool isDown)
    {
        if (ActiveLoop::Has())
        {
    ...
    void
    GameImp::handleMouse(const math::vec2 movement)
    {
        if (ActiveLoop::Has())
        {
