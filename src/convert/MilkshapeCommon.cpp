...
namespace convert
{
    namespace milkshape
    {
        namespace common
        {
            Model::Model() : framecount(0), currentFrame(0)
            {
            }
            ...
            Mesh::Mesh() : flags(0), materialId(0)
            {
            }
            ...
            Vertex::Vertex() : flags(0), uv(0, 0), pos(0, 0, 0), bone(0)
            {
            }
            ...
            Bone::Bone() : name(""), parentName(""), parentId(-1), flags(0)
            {
            }
            ...
            {
                mesh::Animation
                ExtractAnimation(const Model& model)
                {
                    using namespace pwn::mesh;
                    std::vector<AnimationPerBone> ani =
                            std::vector<AnimationPerBone>();

                    bool added = false;

                    BOOST_FOREACH (
                            const milkshape::common::Bone& b,
                            model.bones)
                    {
                        ...
                        BOOST_FOREACH (const RotationKey& rk, b.rotations)
                        {
                            a.addRotation(
                                    rk.time - 1,
                                    makeQuat(math::vec3(rk.x, rk.y, rk.z)));
                            added = true;
                        }
                    ...
                    BOOST_FOREACH (Mesh me, model.meshes)
                    {
                        ...
                        BOOST_FOREACH (Vertex v, me.vertices)
                        {
                            const math::vec4 boneIndex(
                                    ...
                                    -1);
                            builder->addPosition(
                                    pwn::mesh::BPoint(v.pos, boneIndex));
                            builder->addTextCoord(
                                    math::vec2(math::X(v.uv), math::Y(v.uv)));
                            ++vadded;
                        }
                ...
                math::quat
                makeQuat(const math::vec3& angles)
                {
                    real ang;
                    ...
                    math::quat a = math::GetNormalized(
                            math::quat(w, math::vec3(x, y, z)));
                    return a;
                }
            }
        }
