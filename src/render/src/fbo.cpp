...
namespace render
{
    ...
    Fbo::Fbo(int w, int h, bool mipmap) : fbo(0), width(w), height(h)
    {
        ...
        texture.reset(
                new Image(true, width, height, 0, mipmap, GL_RGBA, 1, false));
        ...
        if (status != GL_FRAMEBUFFER_COMPLETE)
            throw static_cast<string>(
                    core::Str()
                    << "Error when creating framebuffer: " << status);

        Bind(0);
    }

    void
    Fbo::bindTexture(int position)
    {
