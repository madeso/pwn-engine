...
namespace component
{
    PropertyDef::PropertyDef(PropertyDef::Type t) : type(t)
    {
    }

    PropertyDef::Type
    PropertyDef::getType() const
    {
        return type;
    }
    ...
    void
    ObjectDef::addProperty(const string& name, const PropertyDef& def)
    {
        properties.insert(Properties::value_type(name, def));
    }

    void
    ObjectDef::addComponent(const ComponentDef& def)
    {
    ...
    namespace
    {
        ...
        CreateProperty(const PropertyDef& def)
        {
            switch (def.getType())
            {
            case PropertyDef::String: return property::CreateString("");
            case PropertyDef::Real: return property::CreateReal(0);
    ...
    ObjectDef::create(const ComponentCreator& creator) const
    {
        ...
        {
            r->addComponent(
                    creator.create(def.name, r->getProperties(), def.args));
        }
