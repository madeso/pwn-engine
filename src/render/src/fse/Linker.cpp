...
namespace render
{
    namespace fse
    {
        ...
        {
            TargetPtr
            CreateTargetFunction(const string& name)
            {
                throw FseException(name + " is not a defined target");
            }

            ProviderPtr
            CreateProviderFunction(const string& name)
            {
                throw FseException(name + " is not a defined provider");
            }

            ShaderPtr
            CreateShaderFunction(const string& name)
            {
                ShaderPtr shader;
                return shader;
            }
        ...
        void
        Linker::addProvider(ProviderPtr prov)
        {
            providers.add(prov->getId(), prov);
        }

        void
        Linker::addTarget(TargetPtr targ)
        {
            targets.add(targ->getId(), targ);
        }

        TargetPtr
        Linker::getTarget(string name)
        {
            return targets.get(name);
        }

        ProviderPtr
        Linker::getProvider(const string& name)
        {
            return providers.get(name);
        }

        void
        Linker::storePipeline(PipelinePtr pl, TargetPtr target)
        {
        ...
        Linker::link()
        {
            ...
            BOOST_FOREACH (TargetPtr t, targets.getData())
            {
                ...
                {
                    throw FseException(
                            t->toString() + " is missing a provider");
                }
            }
        ...
        ShaderPtr
        Linker::getShaderOrNull(const string& id)
        {
            return shaders.get(id);
        }

        const string
        Linker::read(const string& path, ShaderPool* pool)
        {
            core::Ptree file;
            io::Read(path, &file);
            ...
            BOOST_FOREACH (
                    const core::Ptree::value_type& shaderElement,
                    root.get_child("shaders"))
            {
                ...
                if (type == "shader")
                {
                    ...
                    ShaderPtr shader = pool->getFromSource(
                            data.get_child("source"),
                            core::Str() << path << ":" << id);
            ...
            BOOST_FOREACH (
                    const core::Ptree::value_type& targetElement,
                    root.get_child("targets"))
            {
            ...
            BOOST_FOREACH (
                    const core::Ptree::value_type& providerElement,
                    root.get_child("providers"))
            {
