...
namespace pwn
{
	namespace render
	{
		namespace fse
		{
			...
			const string& Target::getId() const
			{
				return id;
			}
			...
			void Target::setProvider(Provider* aprovider)
			{
				...
				{
					throw FseException("failed to set " + aprovider->toString() + " as a provider for " + toString());
				}
			}
			...
			BufferReferencePtr Target::createBuffer(const string& name)
			{
			...
			BufferReferencePtr Target::createBuffer(const string& name, int width, int height)
			{
				associate(name, width, height);
				return createBuffer(name);
			}

			void Target::associate(const string& name, int width, int height)
			{
				associations.insert(AssMap::value_type(name, Size(width, height)));
			}

			const string Target::toString() const
			{
				return getId();
			}
		}
