...
namespace mesh
{
    ...
    Timed::Timed(real t) : time(t)
    {
    }

    FramePosition::FramePosition() : Timed(0), position(math::Origo3().vec)
    {
    }
    ...
    math::vec3
    FramePosition::value() const
    {
        return position;
    }

    string
    FramePosition::toString() const
    {
    ...
    FrameRotation::FrameRotation() : Timed(0), rotation(math::qIdentity())
    {
    }
    ...
    math::quat
    FrameRotation::value() const
    {
        return rotation;
    }

    string
    FrameRotation::toString() const
    {
    ...
    math::quat
    Interpolate(
            const FrameRotation& from,
            real current,
            const FrameRotation& to)
    {
        real scale = math::To01(from.getTime(), current, to.getTime());
        if (math::IsWithinInclusive(0, scale, 1) == false)
        {
    ...
    R
    Get(const std::vector<T>& da, real current)
    {
    ...
    math::quat
    Interpolate(real time, const std::vector<FrameRotation>& fr)
    {
        return Get<FrameRotation, math::quat>(fr, time);
    }

    math::vec3
    Interpolate(real time, const std::vector<FramePosition>& fp)
    {
    ...
    AnimationPerBone::AnimationPerBone(
            std::vector<FramePosition>& afp,
            std::vector<FrameRotation>& afr)
    {
        fp = afp;
        fr = afr;
    }
    ...
    void
    AnimationPerBone::addPosition(real time, const math::vec3& pos)
    {
        fp.push_back(FramePosition(time, pos));
    }

    void
    AnimationPerBone::addRotation(real time, const math::quat& q)
    {
        fr.push_back(FrameRotation(time, q));
    }

    string
    AnimationPerBone::toString() const
    {
        return core::Str() << "<" << fp.size() << " " << fr.size() << ">";
    }

    real
    AnimationPerBone::getLength() const
    {
    ...
    PosePerBone
    AnimationPerBone::getBonePose(real time) const
    {
        const PosePerBone p(Interpolate(time, fp), Interpolate(time, fr));
        return p;
    }
    ...
    void
    AnimationPerBone::sub(int start, int end, AnimationPerBone* out) const
    {
    ...
    void
    AnimationPerBone::scale(real scale)
    {
    ...
    CompiledPose::CompiledPose(const Pose& pose, const std::vector<Bone>& bones)
    {
        ...
        for (std::size_t boneIndex = 0; boneIndex < pose.bones.size();
             ++boneIndex)
        {
            ...
            math::mat44 parent = bone.hasParent()
                    ? result[bone.getParent()].first
                    : math::mat44Identity();
            const math::vec3 poseloc = pose.bones[boneIndex].position;
            const math::quat poserot = pose.bones[boneIndex].rotation;

            const math::mat44 anim =
                    math::mat44helper(math::mat44Identity())
                            .translate(poseloc)
                            .rotate(math::GetConjugate(poserot))
                            .mat;
            const math::mat44 skel =
                    math::mat44helper(math::mat44Identity())
                            .translate(bone.pos)
                            .rotate(math::GetConjugate(bone.rot))
                            .mat;

            result[boneIndex] = CompiledPoseData(
                    math::mat44helper(parent).mult(skel).mult(anim).mat,
                    bone.hasParent() ? bone.getParent() : -1);
        }
        transforms = result;
    }
    ...
    real
    CalculateLength(const std::vector<AnimationPerBone>& bones)
    {
        real length = 0;
    ...
    void
    Animation::getPose(real time, Pose* out) const
    {
    ...
    void
    Animation::subanim(int start, int end, Animation* out) const
    {
    ...
    void
    Animation::subanim(const AnimationInformation& info, Animation* out) const
    {
        subanim(info.start, info.end, out);
    }

    void
    Animation::scale(real scale)
    {
        for (std::size_t i = 0; i < bones.size(); ++i)
        {
