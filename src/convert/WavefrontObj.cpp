	...
	namespace convert
	{
		namespace obj
		{
			namespace // local
			{
				...
				long clong(const pwn::string& str)
				{
				...
				const mesh::BTriangle::Vertex cFaceIndex(const pwn::string& astr, std::size_t pointCount, std::size_t normalCount, std::size_t uvCount)
				{
					...
					const long textureCoordiante = (b == pwn::string::npos) ? clong(str.substr(a+1)) : clong(str.substr(a+1, b-a-1));
					const long normal = (b == pwn::string::npos)?1:clong(str.substr(b+1, str.length()-b-1));
					return mesh::BTriangle::Vertex(FixFaceIndex(vertex, pointCount), FixFaceIndex(normal, normalCount), FixFaceIndex(textureCoordiante, uvCount));
				}
			...
			namespace // local
			{
				...
				void LoadMaterialLibrary(mesh::Builder* converter, std::map<std::string, std::size_t>* materials, const pwn::string& sourceFile, const pwn::string& materialLibraryFileName)
				{
					...
					while( std::getline(f, line) )
					{
						...
						{
							throw (core::Str() << "Unknown material command " << command ).toString();
						}
					}
