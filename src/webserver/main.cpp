...
{
public:
    Request(mg_connection* conn) : conn(conn)
    {
    }

    const std::string
    getVariable(const std::string& name, const std::string& def) const
    {
...
public:
    ...
    virtual void
    response(const Request& req, std::stringstream& stream) = 0;
};

static void
bar(struct mg_connection* conn, const struct mg_requst_info* ri, void* data)
{
...
{
public:
    Server(const std::string& port) : ctx(mg_start())
    {
    ...
    void
    addPage(const std::string& uri, Page* page)
    {
        mg_bind_to_uri(
                ctx,
                uri.c_str(),
                reinterpret_cast<mg_callback_t>(&bar),
                page);
    }
...
public:
    void
    response(const Request& req, std::stringstream& stream)
    {
        stream << "<html><body>"
               << "Hello " << req.getVariable("name", "unknown person")
