...
namespace mesh
{
    ...
    Point::Point(
            const math::vec3& alocation,
            math::vec3 anormal,
            math::vec2 atextcoord,
            math::vec4 abone)
        : position(alocation)
        , normal(anormal)
    ...
    int
    Check(const string& reason, size_t index, size_t size)
    {
        if (index >= size)
        {
    ...
    void
    SetBoneIndex(real* val, BoneIndex newIndex)
    {
    ...
    Mesh::Mesh(
            const std::vector<math::vec3>& posv,
    ...
    pwn::uint32
    Mesh::validate(bool testSortedBones) const
    {
        pwn::uint32 errors = 0;
        ...
        BOOST_FOREACH (const TriangleMap::value_type& mt, triangles)
        {
            ...
            BOOST_FOREACH (const pwn::mesh::Triangle& t, mt.second)
            {
                ...
                {
                    errors +=
                            Check("position index", t[i], vertexes.getCount());
                }
            }
        }
    ...
    pwn::uint32
    Mesh::getNumberOfTriangles() const
    {
        uint32 count = 0;
        for (Mesh::TriangleMap::const_iterator i = triangles.begin();
             i != triangles.end();
             ++i)
        {
            count += i->second.size();
    ...
    VertexData::VertexData(
        ...
        : count(posv.size())
    {
        Assert(posv.size() == normv.size() && textv.size() == bonev.size() &&
               normv.size() ==
                       textv.size());  // if this fails, we didnt get a matched data
    ...
    const VertexData&
    VertexData::operator=(const VertexData& m)
    {
        doCopy(m);
        return *this;
    }
    ...
    pwn::uint32
    VertexData::getCount() const
    {
        return count;
    }

    math::vec4
    VertexData::getBone(pwn::uint32 id) const
    {
        return bone(id);
    }

    const Point
    VertexData::getPoint(uint32 i) const
    {
        return Point(pos(i), norm(i), tex(i), bone(i));
    }

    void
    VertexData::setLocationNormal(
            const uint32 i,
            const math::vec3& apos,
            const math::vec3& anorm)
    {
        pos(i) = apos;
        norm(i) = anorm;
    }
    ...
    math::vec3x
    VertexData::pos(const uint32 i)
    {
        return math::vec3x(&locations[i * 3]);
    }

    math::vec3x
    VertexData::norm(const uint32 i)
    {
        return math::vec3x(&normals[i * 3]);
    }

    math::vec2x
    VertexData::tex(const uint32 i)
    {
        return math::vec2x(&textcoords[i * 2]);
    }

    math::vec4x
    VertexData::bone(const uint32 i)
    {
        return math::vec4x(&boneindexes[i * 4]);
    }

    const math::vec3x
    VertexData::pos(const uint32 i) const
    {
        return math::vec3x(&locations[i * 3]);
    }

    const math::vec3x
    VertexData::norm(const uint32 i) const
    {
        return math::vec3x(&normals[i * 3]);
    }

    const math::vec2x
    VertexData::tex(const uint32 i) const
    {
        return math::vec2x(&textcoords[i * 2]);
    }

    const math::vec4x
    VertexData::bone(const uint32 i) const
    {
        return math::vec4x(&boneindexes[i * 4]);
    }
    ...
    template <typename T>
    void
    Copy(boost::scoped_array<T>& dst,
         const boost::scoped_array<T>& src,
         std::size_t count)
    {
    ...
    void
    VertexData::doCopy(const VertexData& m)
    {
        count = m.count;
